...
export const STAGE_1_PROMPT = (sourceCode: string, requirements: string, musicEnabled: boolean = true) => `
<content>
${sourceCode}
</content>
...
7. 选择性保留并微调界面，去掉无关的菜单和边框，只保留核心区域。鼠标的移动是匀速丝滑的，窗口的弹出是带有物理惯性的。
  目的: 让软件看起来比实际使用时更流畅、更轻盈、更高级
8. 不看“过程”，只看“魔法”：极致压缩操作步骤。例如：刚敲完代码 -> 网站立刻生成；刚输入 Prompt -> 视频立刻出现。省略中间的 Loading 时间。
  目的: 制造“所想即所得”的爽感，强调产品的效率和智能
9. 最后的场景可以生成一个无穷画布的感觉，就是最开始是1个演示，然后是2个，然后是4个，然后是16个，最后丝滑淡出显示产品LOGO/Title
...
export const STAGE_2_PROMPT = (musicEnabled: boolean = true) => `
...
` : `5. 动画自动播放：页面加载后直接开始播放动画，无需用户点击。
`}
6. 采用最小化完整复刻原则，专注在营销而不是100%的产品使用，所以需要考虑什么是noise，应该聚焦什么，可以隐藏什么
7. **必须**包含完整的 HTML 文件内容，包括 <head> 中引入 GSAP 和 Tailwind (CDN)。
